...
def bench(dataset, kind, mu_func, n_graphs, n_samples_mc, n_samples_gvoys, seed=42):
    """
...
def parse_args():
    ...
    parser.add_argument("--n_graphs", type=int, default=20, help="Number of graphs")
    parser.add_argument("--n_nodes", type=int, default=50, help="Number of nodes per graph")
...
if __name__ == "__main__":
    ...
    results_output = f"{folder}/{kernel_kind} (n={n_nodes}) x {n_graphs} | seed={seed}.json"
    with open(results_output, "w") as f:        
        json.dump(results, f, indent=2, default=str)
    
    gram_output = f"{folder}/{kernel_kind} (n={n_nodes}) x {n_graphs} | seed={seed}.pickle"
